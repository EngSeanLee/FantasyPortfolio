...
# 0.5 Motion & the Section 8 3D Pivot — RESOLVED (hybrid, not full R3F)

**Decided.** EngSean revisited this once Phase 2 was done (System Map, Architecture toggle, About areas all shipped, per the recommendation below). The direction taken is deliberately narrower than Section 8's full React Three Fiber waypoint system:

**The contradiction that prompted this:** Section 6 (Motion & Interaction Rules) and Section 8 (3D World Pivot) — both carried over from v2 — specify constant ambient motion and eventually an R3F camera system with waypoints. An earlier pass tried this via an SVG `feTurbulence` cloud-displacement filter over the flat painting; EngSean disliked what it did to the art and it was removed. See the git history / README for that episode.

**What was actually built instead — see `motion-plan.html` (published as a Claude Artifact) for the full plan:** the master painting stays untouched as a single static base plate; every requested behavior (ambient life, nav-tied camera moves, clickable hotspots) is either plain CSS/SVG against the existing image, or a small set of new sprite assets — never a repaint of the painting itself, and never a pixel-displacing filter.

- **Ambient life** — `components/environment/AmbientLife.tsx`: three bird sprites (`public/environment/sprites/*.png`, AI-generated, chroma-keyed transparent, composited cleanly against the sky — verified against the real painting, no fringing) drifting on independent CSS-animated loops inside an SVG using the painting's native `1672×941` coordinate space (`viewBox` + `preserveAspectRatio="xMidYMid slice"`, matching the background image's own `object-cover` crop exactly). A water-shimmer highlight travels along a hand-traced approximate path over the river (`stroke-dasharray`/`stroke-dashoffset` animation, `mix-blend-mode: screen`, additive only — never displaces the underlying pixels). A faint cloud-shadow drifts across the meadow (blurred radial-gradient ellipse, `multiply` blend, very low opacity). All of it returns `null` under `prefers-reduced-motion` — the painting alone still reads as a complete scene with it off.
- **Nav-tied camera moves** — `lib/environment-waypoints.ts` + `LivingEnvironment.tsx`: a per-route `{scale, xPercent, yPercent}` waypoint, animated via Framer Motion (0.9s, matching Section 6's spatial-transition timing) on the single background image. Home is the identity/rest waypoint; Architecture pushes in hardest (toward the Observatory); Projects, About, and Contact each get a lighter, more restrained shift. Forced back to the identity waypoint under reduced motion.
- **Clickable hotspots** — `components/home/EnvironmentHotspots.tsx`, Home-only (`lg:` and up — the one route where the painting is fully visible, not mostly covered by a `GlassPanel`, and the one route whose camera waypoint is always at rest, so painting-space coordinates map onto the viewport without needing to also account for a reframe transform). Two invisible circular regions over features already in the painting — the Observatory (→ `/architecture`) and the foreground ruins (→ `/projects`) — with a soft champagne glow on hover/focus, real `<Link>`s underneath so they're keyboard- and touch-operable even without the hover affordance.
- **`lib/use-cover-rect.ts`** — a small hook mapping painting-space coordinates to real viewport pixels using the same math as CSS `object-cover`, so hotspots (and anything positioned against the painting in the future) line up correctly regardless of viewport aspect ratio. Starts at an empty rect on both server and the client's first render (only computes the real, `window`-derived rect after mount) — a naive `useState(() => computeRect())` initializer was an early hydration-mismatch bug here; worth knowing if this pattern gets reused elsewhere.

**Explicitly not built:** the full Section 8 React Three Fiber depth-layered scene (Path A) and the Blender diorama (Path B) remain out of scope — this hybrid approach delivers the same *felt* result (a world that moves and responds) without the asset-pipeline risk or the R3F dependency. Sections 6 and 8 below stay as historical reference for the fuller vision, not a current build target, unless EngSean asks to revisit the heavier path specifically.

**Shipped on Home first, per the original recommendation** (System Map → Architecture toggle → About areas → *then* motion, reviewed before expanding) — Home is the checkpoint; the ambient layer and camera waypoints are technically live on every route already since they're built into the globally-mounted `LivingEnvironment`, but hotspots stay Home-only by design.
...
## Phase 5 — Motion & Interactivity (Section 0.5 resolved — hybrid approach, not R3F) — **HOME SHIPPED**
Section 0.5 resolved: ambient life (birds, water shimmer, cloud-shadow), nav-tied camera reframing, and clickable hotspots, all built against the untouched static painting rather than the full R3F depth-layered scene. See Section 0.5 for the full breakdown and file list. Ambient life + camera waypoints are live globally (mounted in `LivingEnvironment`); hotspots are Home-only. Next: review on Home, then decide whether hotspots extend to other routes and whether any waypoint framing needs tuning.
